...
public class WMonedasCotizaciones extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<MonedasCotizacionesFiltro> filterBI;
	private BeanItemContainer<MonedasCotizaciones> itemsBIC;
	...
	public WMonedasCotizaciones() {
		...
		try {
			...
			filaFiltroHL.addComponents(paisesCBX, buscarBTN);
			...
			UtilUI.confColumn(itemsGRD.getColumn("numeroMoneda"), "Moneda",
					true, true, false, true, 50);
			UtilUI.confColumn(itemsGRD.getColumn("fecha"), "Fecha", true, 80);
			UtilUI.confColumn(itemsGRD.getColumn("compra"), "Compra", true, 70);
			UtilUI.confColumn(itemsGRD.getColumn("venta"), "Venta", true, 70);
			UtilUI.confColumn(itemsGRD.getColumn("ingreso"), "Ingreso", true,
					120);
			UtilUI.confColumn(itemsGRD.getColumn("usuario"), "Usuario", true,
					100);
			...
			itemsGRD.getColumn("ingreso").setRenderer(
					new DateRenderer(
			...
			order.add(new SortOrder("fecha", SortDirection.ASCENDING));
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<MonedasCotizacionesFiltro>(
				new MonedasCotizacionesFiltro());
		itemsBIC = new BeanItemContainer<MonedasCotizaciones>(
				MonedasCotizaciones.class, new ArrayList<MonedasCotizaciones>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													MonedasCotizaciones item = (MonedasCotizaciones) itemsGRD
															.getSelectedRow();
	...
	private void loadData() {
		try {
			...
			List<MonedasCotizaciones> items = queryData();

			itemsBIC.removeAllItems();

			for (MonedasCotizaciones item : items) {
	...
	private List<Monedas> queryDataMonedas() {
		try {

			return mockDataMonedas();
	...
	private List<MonedasCotizaciones> queryData() {
		try {
			...
			List<MonedasCotizaciones> items = mockData(limit, offset,
	...
	private void deleteItem(MonedasCotizaciones item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getNumeroMoneda()
						.equals(item.getNumeroMoneda())
						&& itemsMock.get(i).getFecha().equals(item.getFecha())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<MonedasCotizaciones> itemsMock = new ArrayList<MonedasCotizaciones>();

	private List<MonedasCotizaciones> mockData(int limit, int offset,
			MonedasCotizacionesFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				MonedasCotizaciones item = new MonedasCotizaciones();

				item.setNumeroMoneda(0);
				item.setFecha(new Date(System.currentTimeMillis()));
				item.setCompra(new BigDecimal(i / 3.36987));
				item.setVenta(new BigDecimal(i / 3.36987));
				item.setIngreso(new Timestamp(System.currentTimeMillis()));
				item.setUsuario("Usuario " + i);
		...
		ArrayList<MonedasCotizaciones> arrayList = new ArrayList<MonedasCotizaciones>();

		for (MonedasCotizaciones item : itemsMock) {

			boolean passesFilterNumeroMoneda = (filtro.getMoneda() == null || item
					.getNumeroMoneda().equals(filtro.getMoneda().getNumero()));

			if (passesFilterNumeroMoneda) {
				arrayList.add(item);
			}
		}
	...
	private List<Monedas> mockDataMonedas() {

		List<Monedas> itemsMock = new ArrayList<Monedas>();
		...
		if (itemsMock.size() == 0) {
			...
			for (int i = 0; i < 500; i++) {
				...
				item.setCotizacion(new BigDecimal(i / 3.36987));
				item.setFecha(new Date(System.currentTimeMillis()));
