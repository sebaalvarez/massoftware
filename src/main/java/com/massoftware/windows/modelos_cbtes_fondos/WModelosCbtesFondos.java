...
public class WModelosCbtesFondos extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<ModelosCbtesFondosFiltro> filterBI;
	private BeanItemContainer<ModelosCbtesFondos> itemsBIC;
	...
	public WModelosCbtesFondos() {
		...
		try {
			...
			UtilUI.confWinList(this, "Modelos cbtes. de fondo");
			...
			UtilUI.confColumn(itemsGRD.getColumn("nombre"), "Nombre", true, -1);
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<ModelosCbtesFondosFiltro>(
				new ModelosCbtesFondosFiltro());
		itemsBIC = new BeanItemContainer<ModelosCbtesFondos>(
				ModelosCbtesFondos.class, new ArrayList<ModelosCbtesFondos>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													ModelosCbtesFondos item = (ModelosCbtesFondos) itemsGRD
															.getSelectedRow();
	...
	private void loadData() {
		try {
			...
			List<ModelosCbtesFondos> items = queryData();

			itemsBIC.removeAllItems();

			for (ModelosCbtesFondos item : items) {
	...
	private List<ModelosCbtesFondos> queryData() {
		try {
			...
			List<ModelosCbtesFondos> items = mockData(limit, offset,
		...
		return new ArrayList<ModelosCbtesFondos>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(ModelosCbtesFondos item) {
		try {
	...
	List<ModelosCbtesFondos> itemsMock = new ArrayList<ModelosCbtesFondos>();

	private List<ModelosCbtesFondos> mockData(int limit, int offset,
			ModelosCbtesFondosFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				ModelosCbtesFondos item = new ModelosCbtesFondos();
		...
		ArrayList<ModelosCbtesFondos> arrayList = new ArrayList<ModelosCbtesFondos>();

		for (ModelosCbtesFondos item : itemsMock) {
