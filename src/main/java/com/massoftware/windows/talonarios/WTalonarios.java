...
public class WTalonarios extends Window {
	...
	public WTalonarios() {
		...
		try {
			...
			itemsGRD.setWidth("480px");

			itemsGRD.setColumns(new Object[] { "numero", "nombre", "letra",
			...
			UtilUI.confColumn(itemsGRD.getColumn("letra"), "Letra", true, 70);
			UtilUI.confColumn(itemsGRD.getColumn("sucursal"), "Sucursal", true,
					70);
	...
	private void buildContainersItems() throws Exception {
		...
		itemsBIC = new BeanItemContainer<Talonarios>(Talonarios.class,
				new ArrayList<Talonarios>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													Talonarios item = (Talonarios) itemsGRD
															.getSelectedRow();
	...
	private List<Talonarios> queryData() {
		try {
			...
			List<Talonarios> items = mockData(limit, offset,
	...
	private List<Talonarios> mockData(int limit, int offset,
			TalonariosFiltro filtro) {
