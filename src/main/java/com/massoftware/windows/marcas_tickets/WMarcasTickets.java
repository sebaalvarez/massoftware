...
public class WMarcasTickets extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<MarcasTicketsFiltro> filterBI;
	private BeanItemContainer<MarcasTickets> itemsBIC;
	...
	public WMarcasTickets() {
		...
		try {
			...
			UtilUI.confWinList(this, "Marcas de tickets");
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<MarcasTicketsFiltro>(new MarcasTicketsFiltro());
		itemsBIC = new BeanItemContainer<MarcasTickets>(MarcasTickets.class,
				new ArrayList<MarcasTickets>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													MarcasTickets item = (MarcasTickets) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				MarcasTickets item = (MarcasTickets) itemsGRD.getSelectedRow();
				item.getNumero();
	...
	private void loadData() {
		try {
			...
			List<MarcasTickets> items = queryData();

			itemsBIC.removeAllItems();

			for (MarcasTickets item : items) {
	...
	private List<MarcasTickets> queryData() {
		try {
			...
			List<MarcasTickets> items = mockData(limit, offset,
	...
	private void deleteItem(MarcasTickets item) {
		try {
	...
	private List<MarcasTickets> mockData(int limit, int offset,
			MarcasTicketsFiltro filtro) {
		...
		ArrayList<MarcasTickets> arrayList = new ArrayList<MarcasTickets>();

		for (MarcasTickets item : itemsMock) {
