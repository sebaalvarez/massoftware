...
public class WCajas extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<CajasFiltro> filterBI;
	private BeanItemContainer<Cajas> itemsBIC;
	...
	private void buildContainersItems() throws Exception {
		...
		itemsBIC = new BeanItemContainer<Cajas>(Cajas.class,
				new ArrayList<Cajas>());
	}
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				Cajas item = (Cajas) itemsGRD.getSelectedRow();
				item.getNumero();
	...
	private void loadData() {
		try {
			...
			List<Cajas> items = queryData();

			itemsBIC.removeAllItems();

			for (Cajas item : items) {
	...
	private List<Cajas> queryData() {
		try {
			...
			List<Cajas> items = mockData(limit, offset, this.filterBI.getBean());
	...
	private void deleteItem(Cajas item) {
		try {
	...
	private List<Cajas> mockData(int limit, int offset, CajasFiltro filtro) {
		...
		ArrayList<Cajas> arrayList = new ArrayList<Cajas>();

		for (Cajas item : itemsMock) {
