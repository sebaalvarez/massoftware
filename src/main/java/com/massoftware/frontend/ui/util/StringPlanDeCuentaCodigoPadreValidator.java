...
public class StringPlanDeCuentaCodigoPadreValidator extends
	...
	public StringPlanDeCuentaCodigoPadreValidator(String errorMessage,
	...
	protected boolean isValidValue(String codigoCuentaPadre) {
		...
		if (codigoCuentaPadre != null && codigoCuentaPadre.equals("00000000000") == false) {
			try {
...
//						chars[7], chars[8], chars[9], chars[10]);
				...
				String codigoCuentaPadre2 = FormatPlanDeCuentaCodigoCuenta.format(codigoCuentaPadre);

				this.setErrorMessage("El campo Integra es incorrecto, no existe una cuenta padre con la denominación ("
						+ planDeCuentaBI.getBean().getEjercicioContable() + " - " + codigoCuentaPadre2 + ")");

				return cx.buildPlanDeCuentaBO().ifExistsCodigoCuentaPadre(
						codigoCuentaPadre, planDeCuentaBI.getBean().getEjercicioContable().getEjercicio());
			} catch (Exception e) {
